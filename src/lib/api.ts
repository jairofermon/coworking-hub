...
export async function fetchPlanos(): Promise<Plano[]> {
  ...
  if (error) throw error;
  return (data ?? []).map(r => ({ id: r.id, nome: r.nome, descricao: r.descricao ?? '', valor_previsto: Number(r.valor_previsto), ativo: r.ativo }));
}

export async function upsertPlano(p: Omit<Plano, 'id'> & { id?: string }): Promise<Plano> {
  const payload = { nome: p.nome, descricao: p.descricao, valor_previsto: p.valor_previsto, ativo: p.ativo };
  if (p.id) {
    const { data, error } = await supabase.from('planos').update(payload).eq('id', p.id).select().single();
    if (error) throw error;
    return { id: data.id, nome: data.nome, descricao: data.descricao ?? '', valor_previsto: Number(data.valor_previsto), ativo: data.ativo };
  }
